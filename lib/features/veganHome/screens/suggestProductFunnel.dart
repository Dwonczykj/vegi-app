...
class _SuggestProductFunnelScreenState extends State<SuggestProductFunnelScreen>
  ...
  final List<ProductSuggestionImageType> screenTypes =
      <ProductSuggestionImageType>[
  ...
  void initState() {
    super.initState();
    // screenTypes = [
    //   ...(widget.scannedQRCode != null ?
    //     [] :
    //     <ProductSuggestionImageType>[ProductSuggestionImageType.barCode]),
    //   ...<ProductSuggestionImageType>[
    //     ProductSuggestionImageType.frontOfPackage,
    //     ProductSuggestionImageType.nutritionalInfo,
    //     ProductSuggestionImageType.ingredientInfo,
  ...
  bool isLastPage(int screenCount) =>
      _pageController.positions.isNotEmpty &&
      _pageController.page!.toInt() >= screenCount;

  bool isFirstPage() =>
      _pageController.positions.isEmpty || _pageController.page!.toInt() == 0;

  void Function() nextPage(int screenCount) {
    final pageCount = screenCount;
    return () {
  ...
  void Function() previousPage(int screenCount) {
    return () {
          ...
          if (widget.scannedQRCode != null) {
            viewmodel.addProductQRCodeForProductSuggestion(
              ...
              (errMessage, errCode) {
                ...
                showErrorSnack(
                  context: context,
                  message: 'Unable to send barcode to vegi',
                );
              },
            );
            gotoPage(1);
          ...
          return Scaffold(
            appBar: CustomAppBar(
              pageTitle: _pageController.hasClients &&
                      _pageController.positions.isNotEmpty
                  ? _label(screenTypes[_pageController.page!.round()])
                      .capitalize()
                                      ...
                                      PageView.builder(
                                        ...
                                        itemBuilder:
                                            (BuildContext context, int index) =>
                                                suggestionScreens[index %
                                                    suggestionScreens.length],
                                      ),
                                      Positioned(
                                        bottom: 25,
                                        left:
